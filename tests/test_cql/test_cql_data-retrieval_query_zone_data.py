...
for collection_name, collection in collections_dict.items():
    ...
    if (cp.datasources[datasource_id].datetime_col is not None):
        # because zone data retrieval separates the handling for temporal/non-temporal collections
        # and we are not testing the datetime parameter here, so it is default to non-temporal collections
        continue
...
def test_cql_data_retrieval():
    ...
    for dggrsid, validation_set in validation_df.items():
        ...
        for cql_string, zid in zip(cql_list, test_zoneid):
            ...
            if ("<" in cql_string):
                value = np.nanmax(tdata[column_names[0]].values)
                if ("=" in cql_string):
                    tdata = tdata[tdata[column_names[0]] <= value]
                else:
                    tdata = tdata[tdata[column_names[0]] < value]
            elif (">" in cql_string):
                value = np.nanmin(tdata[column_names[0]].values)
                if ("=" in cql_string):
                    tdata = tdata[tdata[column_names[0]] >= value]
                else:
                    tdata = tdata[tdata[column_names[0]] > value]
