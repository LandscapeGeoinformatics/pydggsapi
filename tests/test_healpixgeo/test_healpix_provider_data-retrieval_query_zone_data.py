...
for collection_name, collection in collections_dict.items():
    ...
    if ("nested" in collection.collection_provider.dggrsId):
        textual_non_exist_zone_ids = list(map(lambda x: f"{str(rf).zfill(2)}_{str(x)}", non_exist_zone_ids))
    else:
...
def test_healpixgeo_data_retrieval():
    ...
    for collection_name, df_dict in validation_df.items():
        ...
        response = client.get(f"/dggs-api/dggs/{dggrsid}/zones/{zone['zone_id']}/data", params={"zone-depth": over_rf_depth})
        ...
        response = client.get(f'/dggs-api/dggs/{dggrsid}/zones/{zone["zone_id"]}/data')
        ...
        response = client.get(f'/dggs-api/dggs/{dggrsid}/zones/{zone["zone_id"]}/data', headers={'accept': 'application/geo+json'},
                              params={'zone-depth': '0'})
        assert response.status_code == 200
        ...
        response = client.get(f'/dggs-api/dggs/{dggrsid}/zones/{zone["zone_id"]}/data', params={'geometry': 'zone-centroid', 'zone-depth': '0'},
        ...
        response = client.get(f'/dggs-api/dggs/{dggrsid}/zones/{zone["zone_id"]}/data', params={'zone-depth': '2'})
        data = ZonesDataDggsJsonResponse(**response.json())
        assert response.status_code == 200
        for k, v in data.values.items():
            assert len(v) == 1
        ...
        response = client.get(f'/dggs-api/dggs/{dggrsid}/zones/{zone["zone_id"]}/data', params={'zone-depth': '1-2'})
        data = ZonesDataDggsJsonResponse(**response.json())
        assert response.status_code == 200
        ...
        response = client.get(f'/dggs-api/dggs/{dggrsid}/zones/{zone["zone_id"]}/data', params={'zone-depth': '0-2'})
        data = ZonesDataDggsJsonResponse(**response.json())
        assert response.status_code == 200
        ...
        response = client.get(f'/dggs-api/dggs/{dggrsid}/zones/{non_exists_zoneid}/data',
                              params={'zone-depth': '0-2', 'geometry': 'zone-centroid'},
                              headers={'accept': 'application/geo+json'})
